...
class MPNEncoder(nn.Module):
    def __init__(self, args: Namespace, atom_fdim: int, bond_fdim: int):
        ...
        for depth in range(self.depth-1):
            self._modules[f'W_h_bond_{depth}'] = nn.Linear(w_h_input_size_bond, self.hidden_size, bias=self.bias)
    ...
    def forward(self,mol_graph: BatchMolGraph, features_batch=None) -> torch.FloatTensor:
        ...
        for depth in range(self.depth - 1):
            ...
            message_atom = self.act_func(self._modules[f'W_h_bond_{depth}'](message_atom))
            message_bond = self._modules[f'W_h_bond_{depth}'](message_bond)
            message_bond = self.act_func(input_bond + message_bond)
            message_bond = self.dropout_layer(message_bond)  # num_bonds x hidden
        
        # communicate
        agg_message = index_select_ND(message_bond, a2b)
        agg_message = agg_message.sum(dim=1) * agg_message.max(dim=1)[0]
        
        agg_message = self.communicate_mlp(torch.cat([agg_message, message_atom, input_atom], 1))
# =============================================================================
#         
# =============================================================================
        # readout
        a_message = self.gru(agg_message, a_scope)
